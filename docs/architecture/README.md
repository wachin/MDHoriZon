...
| [0007](0007-theming.md)                                   | One palette in one file using the roadmap's variable names; the theme resolved in JavaScript onto `data-theme`; a persisted manual choice; and WCAG AA contrast asserted by test in both schemes                                                  |
| [0008](0008-assets-and-the-content-package.md)            | Relative asset URLs are resolved at render time against the document's own location, never rewritten into the document; a content package is a document plus the assets its relative paths reach, stored package-relative so a copy keeps working |
| [0009](0009-content-model.md)                             | Identity comes from the folder tree: `id` from the path, language/program/date from the folders, frontmatter optional and winning when present, and legacy Blogger markup rescued in the text                                                     |
| [0010](0010-media-and-embeds.md)                          | Videos: the cover is YouTube's own thumbnail (needs no player), legacy embeds are rescued as links, and playing in place is a click-to-play facade that contacts YouTube only when the reader presses play                                        |
