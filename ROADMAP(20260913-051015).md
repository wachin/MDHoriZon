...
# Future Vision — From Reader to Markdown Editor

If MDHoriZon becomes a successful real-world application and the rendering architecture proves reliable, a future experiment will be to investigate whether the same foundation can evolve into a full Markdown editor.

The long-term idea is to explore an editor experience inspired by applications such as:

- Typora.
- VNote.
- Obsidian.

The important point is that this should be an **experiment after the reader is mature**, not something that complicates the first implementation.
...
```text
              ...
              ┌────────────┴────────────┐
              ▼                         ▼
       Markdown Renderer          Markdown Editor
              │                         │
              └────────────┬────────────┘
                           ▼
...
Potential future editor experiments:

- [ ] Source Markdown editor.
- [ ] Live preview.
- [ ] Typora-style rendered editing.
- [ ] Split editor/preview mode.
- [ ] Obsidian-like document navigation.
- [ ] VNote-like note organization.
- [ ] Markdown shortcuts.
- [ ] Live KaTeX.
- [ ] Live Mermaid.
- [ ] Image insertion.
...
- [ ] Export/publishing using the same rendering engine.

The key experiment would be to determine whether **the renderer that powers MDHoriZon's reading experience can also become the rendering foundation of a high-quality Markdown editor**.

If that experiment succeeds, MDHoriZon could eventually evolve from a Markdown reader/publisher into a broader Markdown workspace while preserving the original principle: **one reliable Markdown core shared across web, Android, and iOS.**

---

## Final Principle

> **Build the renderer first. Make it secure. Test it thoroughly. Make it excellent on the web. Then make the same renderer work offline and inside mobile applications. Only after that should MDHoriZon attempt to become an editor.**
